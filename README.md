...
* [`src/cljs/billing_cycle_calculator/`](src/cljs/billing_cycle_calculator/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/cljs/billing_cycle_calculator/core.cljs): contains the SPA entry point, `init`
...
7. Clone this repo and open a terminal in the `billing-cycle-calculator` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/billing_cycle_calculator/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns billing-cycle-calculator.example
  (:require [billing-cycle-calculator.config :as config])
